...
from open_predictive_coder import ByteCodec, BridgeFeatureArrays, BridgeFeatureConfig, bridge_feature_arrays, ensure_tokens
...
class BridgeProxyTrace:
    ...
    @property
    def steps(self) -> int:
        return int(self.base_probs.shape[0])


@dataclass(frozen=True)
class BridgeProxyReport:
    tokens: int
    steps: int
    mean_entropy: float
    mean_peak: float
...
class BridgeProxyModel:
    def __init__(self, config: BridgeProxyConfig | None = None):
        self.config = config or BridgeProxyConfig()
        ...
        self.recurrent = _normalized_matrix(rng, self.config.hidden_dim, self.config.hidden_dim, scale=0.8)
        self.base_head = _normalized_matrix(rng, self.config.vocabulary_size, self.config.hidden_dim, scale=0.9)
        self.proxy_head = _normalized_matrix(rng, self.config.vocabulary_size, self.config.hidden_dim, scale=0.9)
        self.bridge_bias = rng.normal(loc=0.0, scale=0.05, size=(self.config.vocabulary_size,)).astype(np.float64)
    ...
    def _scan_probabilities(self, tokens: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        for token in tokens:
            ...
            base_logits = (self.base_head @ hidden) + self.bridge_bias
            proxy_state = np.mean(np.vstack(window), axis=0)
            proxy_logits = (self.proxy_head @ proxy_state) + self.bridge_bias

            base_probs.append(_softmax(base_logits))
            proxy_probs.append(_softmax(proxy_logits))

        if not base_probs:
            empty = np.zeros((0, self.config.vocabulary_size), dtype=np.float64)
            return empty, empty
        return np.vstack(base_probs), np.vstack(proxy_probs)
    ...
    def trace(
        ...
        base_probs, proxy_probs = self._scan_probabilities(tokens[:-1])
        features = bridge_feature_arrays(base_probs, proxy_probs, self.config.vocabulary_size, config=self.config.bridge)
        return BridgeProxyTrace(
    ...
    def feature_summary(self, data: str | bytes | bytearray | memoryview | np.ndarray | Sequence[int] | Sequence[object]) -> dict[str, float]:
    ...
    def decode_sample(self, text: str) -> np.ndarray:
        return ByteCodec.encode_text(text)
